...
>
> Most users should start with the [Azure MCP](https://holmesgpt.dev/data-sources/builtin-toolsets/azure-mcp/) integration, which provides broad access to all Azure APIs including AKS node diagnostics. This standalone toolset is only needed if you require specific AKS node health CLI commands that aren't available through the MCP server.
